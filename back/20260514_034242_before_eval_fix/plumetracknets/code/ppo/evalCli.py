...
### UTILITY ###
def load_trusted_checkpoint(model_fname):
    """Load project checkpoints saved as full Policy objects.

    PyTorch 2.6 changed torch.load's default to weights_only=True, which rejects
    full-object checkpoints like the ones this project writes.
    """
    try:
        return torch.load(
            model_fname,
        ...
        return torch.load(model_fname, map_location=torch.device('cpu'))
...
if __name__ == "__main__":
    ...
    actor_critic, ob_rms = load_trusted_checkpoint(args.model_fname)
