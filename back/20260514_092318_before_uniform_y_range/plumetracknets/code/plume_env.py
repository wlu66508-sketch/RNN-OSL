...
class PlumeEnvironment(gym.Env):
  ...
  def __init__(self, 
    ...
    dataset_pool=None,
    random_dataset_per_episode=False,
    ...
    self.dataset_pool = self._normalize_dataset_pool(dataset_pool)
    self.random_dataset_per_episode = bool(
        random_dataset_per_episode and len(self.dataset_pool) > 0)
    self._dataset_cache = {}
    ...
    initial_dataset = self.dataset_pool[0] if self.random_dataset_per_episode else dataset
    self.set_dataset(initial_dataset)
  ...
  def _normalize_dataset_pool(self, dataset_pool):
    if dataset_pool is None:
        return []
    if isinstance(dataset_pool, str):
        dataset_pool = dataset_pool.replace(',', ' ').split()
    return [dataset for dataset in dataset_pool if dataset]

  def _dataset_cache_key(self, dataset):
    return (
        dataset,
        float(self.t_val_min),
        float(self.t_val_max),
        float(self.dt),
        float(np.clip(self.birthx_max, a_min=0.01, a_max=1.00)),
        float(self.diffusion_max),
        float(self.radiusx),
    )

  def set_dataset(self, dataset, verbose=True):
    self.dataset = dataset
    cache_key = self._dataset_cache_key(self.dataset)
    if cache_key not in self._dataset_cache:
        self._dataset_cache[cache_key] = sim_analysis.load_plume(
    ...
    self.data_puffs_all, self.data_wind_all = self._dataset_cache[cache_key]
    if self.random_dataset_per_episode and not verbose:
        self.data_puffs = self.data_puffs_all
  ...
  def sample_dataset_for_episode(self):
    dataset = np.random.choice(self.dataset_pool)
    self.set_dataset(dataset, verbose=False)
  ...
  def reset(self):
    ...
    """
    if self.random_dataset_per_episode:
        self.sample_dataset_for_episode()
