...
def build_dataset_pool_for_rank(args, rank):
    if args is None or not getattr(args, 'random_dataset_per_episode', False):
        return None

    if getattr(args, 'stratified_dataset_sampling', False):
        scenarios = getattr(args, 'scenario_names', None) or []
        seed_start = getattr(args, 'train_seed_start', None)
        seed_end = getattr(args, 'train_seed_end', None)
        if not scenarios or seed_start is None or seed_end is None:
            return None

        num_processes = max(1, getattr(args, 'num_processes', len(scenarios)))
        scenario_idx = min(
            len(scenarios) - 1,
            int(rank * len(scenarios) / num_processes),
        )
        scenario = scenarios[scenario_idx]
        suffix = getattr(args, 'dataset_suffix', 'x20b5_s')
        return [
...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, args=None):
    ...
    def _thunk():
        if 'plume' in env_id:
            dataset_pool = build_dataset_pool_for_rank(args, rank)
            dataset = dataset_pool[0] if dataset_pool else args.dataset
            random_dataset_per_episode = bool(dataset_pool)
            dataset_kwargs = {}
            if not args.dynamic:
                dataset_kwargs = {
                    'dataset_pool': dataset_pool,
                    'random_dataset_per_episode': random_dataset_per_episode,
                }
